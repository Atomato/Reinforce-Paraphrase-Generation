...
JOSA_WITHSPACE=re.compile(u" \(이\)가| \(와\)과| \(을\)를| \(은\)는| \(아\)야| \(이\)여| \(으\)로| \(이\)라")

JOSA_WITHNUM=re.compile(u"\d\(이\)가|\d\(와\)과|\d\(을\)를|\d\(은\)는|\d\(아\)야|\d\(이\)여|\d\(으\)로|\d\(이\)라")

NUM_PAIRED = [('0', '▥영'), ('1','▥일'), ('2', '▥이'),('3','▥삼'),('4','▥사'), ('5','▥오'), ('6','▥육'), ('7','▥칠'), ('8','▥팔'), ('9','▥구')]
...
def remove_spacing(src):
    space_list = JOSA_WITHSPACE.findall(src)
    if space_list == []:
        return src
    else:
        space_list.sort(key=len)
        space_list.reverse()
        for word in space_list:
            src = src.replace(word, word.strip())
        return src
def replace_num2kor(src):
    num_list = JOSA_WITHNUM.findall(src)
    if num_list == []:
        return src
    else:
        ...
        for word in num_list:
            src = src.replace(word, word.replace(word[0], NUM_PAIRED[int(word[0])][1]))
        return src
def replace_kor2num(src):
    for num,kor in NUM_PAIRED:
        src=src.replace(kor, num)
    return src

def replace_josa_org(src):
    tokens = []
    base_index = 0
...
def replace_josa(src):
    return replace_kor2num(replace_josa_org(replace_num2kor(remove_spacing(src))))
